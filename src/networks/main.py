...
from .mlp import MLP, MLP_Autoencoder, MLP_Classifier 
...
def build_network(net_name, ae_net=None):
    ...
    if net_name == 'cardio_mlp':
        net = MLP(x_dim=21, h_dims=[32, 32], rep_dim=32, bias=False)

    if net_name == 'satellite_mlp':
        net = MLP(x_dim=36, h_dims=[32, 32], rep_dim=32, bias=False)

    if net_name == 'satimage-2_mlp':
        net = MLP(x_dim=36, h_dims=[32, 32], rep_dim=32, bias=False)

    if net_name == 'shuttle_mlp':
        net = MLP(x_dim=9, h_dims=[32, 32], rep_dim=32, bias=False)

    if net_name == 'thyroid_mlp':
        net = MLP(x_dim=6, h_dims=[32, 32], rep_dim=32, bias=False)
...
def build_classifier(net_name):
    ...
    '''
    if net_name == 'arrhythmia_mlp':
        cls_net = MLP_Classifier(x_dim=32, h_dims=[32, 16], rep_dim=2, bias=False)
